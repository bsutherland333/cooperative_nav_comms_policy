...
class ActorEncoder:
    """Encode one ordered pair relation for the shared binary actor."""
    ...
    def __init__(
        ...
        self.fleet_estimate_size = num_agents * vehicle_state_size
        self.estimate_size = 2 * vehicle_state_size
    ...
    def encode_state(
        ...
        """Encode one acting-agent/candidate-partner relation."""
        self._validate_agent_id(agent_id)
        self._validate_agent_id(partner_id)
        if agent_id == partner_id:
            raise ValueError("Actor pair encoding requires two different agents.")
        ...
        state_order = self._pair_state_order(agent_id=agent_id, partner_id=partner_id)
        ...
        return jnp.concatenate(
            (
                ordered_mean,
                covariance_features,
                jnp.asarray([time_since_last_communication[partner_id]]),
            )
        )
    ...
    def _state_size(self) -> int:
        ...
        return fleet_belief_size + 1

    def _pair_state_order(self, agent_id: int, partner_id: int) -> jnp.ndarray:
        state_indices = [
            agent_state_index
            for ordered_agent_id in (agent_id, partner_id)
    ...
    def _validate_belief_shapes(
        ...
        if mean.shape != (self.fleet_estimate_size,):
            raise ValueError("Local belief estimate must match fleet state size.")
        if covariance.shape != (self.fleet_estimate_size, self.fleet_estimate_size):
...
class CriticEncoder:
    ...
    def __init__(
        ...
        self.single_belief_state_size = self._single_belief_state_size()
        self.state_size = num_agents * self.single_belief_state_size
    ...
    def _encode_local_belief(self, local_belief: Any, agent_id: int) -> jnp.ndarray:
        mean = jnp.asarray(local_belief.estimate)
        covariance = jnp.asarray(local_belief.covariance)
        time_since_last_communication = jnp.asarray(
            local_belief.time_since_last_communication
    ...
    def _single_belief_state_size(self) -> int:
        if self.encoding_method == StateEncodingMethod.MEAN_DIAGONAL:
            fleet_belief_size = self.estimate_size * 2
        else:
            fleet_belief_size = (
